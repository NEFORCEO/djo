...
2. Иначе, если в самой сигнатуре хендлера есть типовые аннотации у параметров помимо `request`/`self`/path-параметров, djo читает их напрямую — см. [Типизированная сигнатура хендлера](query-parameters.md#типизированная-сигнатура-хендлера).
